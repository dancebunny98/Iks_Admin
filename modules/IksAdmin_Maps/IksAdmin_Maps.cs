...
public class MapEntry
{
    ...
    // Необязательное поле.
    // Если указано — используется host_workshop_map.
    // Если не указано — обычный changelevel.
...
public class Main : AdminModule, IPluginConfig<MapsConfig>
{
    ...
    public void OnConfigParsed(MapsConfig config)
    {
        Config = config;
    }
        ...
        IDynamicMenu? backMenu = null)
    {
        ...
        {
            caller.Print(
                $" {ChatColors.Red}{Localizer["Chat.NoMapsConfigured"]}"
            );
        ...
        IDynamicMenu? backMenu)
    {
        ...
        if (category.Maps.Count == 0)
        {
            menu.AddMenuOption(
                "none",
                Localizer["Chat.NoMapsConfigured"],
                (_, _) => { },
                disabled: true
            );
        }
        ...
        MapEntry map)
    {
        caller.Print(
            $" {ChatColors.Green}" +
            $"{Localizer["Chat.ChangingMap"]} " +
            $"{ChatColors.Gold}{map.Label} ({map.Map})"
        );

        if (Config.AnnounceToAll)
        {
            Server.PrintToChatAll(
                $" {ChatColors.Green}[Maps] " +
                $"{ChatColors.White}{caller.PlayerName} " +
                $"{Localizer["Chat.PlayerChangedMap"]} " +
                $"{ChatColors.Gold}{map.Label}"
            );
        }

        // Если WorkshopId указан — запускаем Workshop-карту.
        if (!string.IsNullOrWhiteSpace(map.WorkshopId))
        {
            Server.ExecuteCommand(
                $"host_workshop_map {map.WorkshopId}"
            );

            return;
        }

        // Если WorkshopId отсутствует — обычная карта.
        Server.ExecuteCommand(
            $"changelevel {map.Map}"
        );
    }
